 ...
 */
async function buildPersonalFeed(
	feedXml: string,
...
async function handleFeed(
	...
	if (!feedUrl) {
		return new Response('Feed not configured', { status: 500 });
	}
	...
	try {
		...
		if (!res.ok) {
			return new Response(`Upstream feed error: ${res.status}`, { status: 502 });
		}
	...
	const personalFeed = await buildPersonalFeed(
		feedXml,
		workerBaseUrl,
		userToken,
		env.SIGNING_KEY,
